...
def get_fare_markup(user:UserDetails):
    fare_obj = OrganizationFareAdjustment.objects.filter(organization = user.organization,module = 'buses').first()
    fare = {
    ...
    if user.role.name == "distributor_agent":
        dafa_obj = DistributorAgentFareAdjustment.objects.filter(user = user,module = 'bus').first()

...
from vendors.flights.stt_travelopedia.api  import (flight_search,fare_rule,fare_quote,
...
import time
import json
from vendors.flights.stt_travelopedia import assistant_manager as AM
...
class Manager(AbstractFlightManager):
    ...
    def get_fare_details(self,**kwargs):
        try:
            ...
            for fare_index,fare_detail in enumerate(sorted_price_list):
                ...
                result['fare_rule'] = "No Fare Rules Available!"
                result['currency'] = "INR"
                result['colour'] = "RED"
    ...
    def find_ticket_status(self,api_response,ticketing_type):
        try:
            ticket_status =  int(api_response["AirPNRDetails"][0]["Ticket_Status_Id"])
            if ticketing_type == 0:
                if ticket_status in [3,4,7]: # InProgress,Confirmed,Blocked
                    return True
                else:
                    return False
            else:
    ...
    def purchase(self,**kwargs):
        ...
        try:
            ...
            if ticket_book_temp["status"] == True:
                ...
                if ticket_booking_details["status"] == True :
                    itinerary.status = "Confirmed"
                    ...
                    airline_pnr,gds_pnr = self.find_pnr(air_reprint_details["data"])
                    ticket_status = self.find_ticket_status(air_reprint_details["data"],int(ticketing_type))
                    if not isLCC:

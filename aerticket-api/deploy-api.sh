...
# Configuration for eth-api
SERVICE_NAME="eth-api"
ECR_REPO_NAME="aerticket-api"
K8S_DEPLOYMENT_NAME="aerticket-api"
...
K8S_DEPLOYMENT_FILE="/Users/riz/aws-eks-infrastructure/k8s/aerticket/api-deployment.yaml"
...
show_help() {
    ...
    echo ""
    echo "Deploy eth-api service to EKS"
    echo ""
    echo "Options:"
    ...
    echo "Examples:"
    echo "  $0 --branch main"
    echo "  $0 --branch feature/new-api --tag v1.2.3"
...
show_deployment_status() {
    log_header "ETH-API Deployment Status"
...
check_prerequisites() {
    log_info "Checking prerequisites for eth-api deployment..."
    
    # Check if we're in the right directory
    if [[ ! -f "$SCRIPT_DIR/Dockerfile" ]]; then
        log_error "Dockerfile not found in $SCRIPT_DIR"
        log_error "Make sure you're running this script from the eth-api directory"
        exit 1
    fi
...
pull_code() {
    ...
    log_info "Pulling latest code for eth-api from branch $BRANCH..."
    ...
    if [[ "$DRY_RUN" == "false" ]]; then
        ...
        if [[ -d ".git" ]]; then
            ...
            if git branch -r | grep -q "origin/$BRANCH"; then
                ...
                log_success "Updated eth-api to latest $BRANCH"
            else
                log_error "Branch $BRANCH not found for eth-api"
                exit 1
            fi
        else
            log_warning "eth-api is not a git repository, skipping git pull"
        fi
    else
        log_info "[DRY RUN] Would pull latest code for eth-api from branch $BRANCH"
    fi
}
...
build_and_push() {
    ...
    log_info "Building and pushing Docker image for eth-api..."
    ...
    if [[ "$DRY_RUN" == "false" ]]; then
        ...
        log_success "Successfully built and pushed eth-api image"
    else
...
update_k8s_deployment() {
    ...
    log_info "Updating Kubernetes deployment for eth-api..."
    ...
    if [[ "$DRY_RUN" == "false" ]]; then
        ...
        log_success "Successfully updated deployment for eth-api"
    else
...
restart_deployment() {
    log_info "Restarting deployment for eth-api..."
    
    if [[ "$DRY_RUN" == "false" ]]; then
        kubectl rollout restart deployment/$K8S_DEPLOYMENT_NAME -n $K8S_NAMESPACE
        kubectl rollout status deployment/$K8S_DEPLOYMENT_NAME -n $K8S_NAMESPACE --timeout=300s
        log_success "Successfully restarted deployment for eth-api"
    else
...
main() {
    ...
    log_success "ETH-API deployment process completed successfully!"
}

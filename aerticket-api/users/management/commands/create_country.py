...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if len(Country.objects.all())>0:
            self.stdout.write(f'Deleting the existing countries')
            [country.hard_delete()for country in Country.objects.all()]
        
        for country in country_details:
            country_obj = Country.objects.create(country_name=country["Country"],country_code=country["CountryCode"],currency_name=country["Currency"],currency_code=country["Code"])
            country_obj.calling_code =self.get_calling_code(country_obj.country_code) if self.get_calling_code(country_obj.country_code) is None else f"+{self.get_calling_code(country_obj.country_code)}"
            country_obj.save()
        self.stdout.write(f'Created Countries')
    ...
    def get_calling_code(self,country_code):
        try:
            ...
            if country is None:
                return None
            phone_metadata = phonenumbers.region_code_for_country_code(phonenumbers.country_code_for_region(country.alpha_2))
            if phone_metadata is None:
                return None
            return phonenumbers.country_code_for_region(country.alpha_2)
        except KeyError:
            return None

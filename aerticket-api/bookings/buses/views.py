...
class BusQueuesView(APIView):
    def get(self, request):
        ...
        if booking_status == "confirmed":
            filter_condition &= Q(status__in=["Confirmed","Rejected","Cancel-Ticket-Failed"])
        elif booking_status == "failed":
            filter_condition &= (Q(status="Ticketing-Failed"))
        elif booking_status == "cancelled":
            filter_condition &= Q(status__in=["Cancelled","Cancel-Ticket-Initiated"])
        elif booking_status == "others":
            filter_condition &= Q(status__in=["Enquiry", "Ticketing-Initiated","Cancel-Ticket-Initiated"])
        ...
        query_set = BusBooking.objects.filter(filter_condition).order_by('-display_id')
